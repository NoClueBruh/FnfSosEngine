...
{
	public var occupiedKeys:Array<FlxKey> = [];
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		{
			case Solo:
				inline bindKeys(Control.UP, getSaveKeys(Control.UP));
				inline bindKeys(Control.DOWN, getSaveKeys(Control.DOWN));
				inline bindKeys(Control.LEFT, getSaveKeys(Control.LEFT));
				inline bindKeys(Control.RIGHT, getSaveKeys(Control.RIGHT));
		...
		{
			case Solo:
			 	bindKeys(Control.UP, getSaveKeys(Control.UP));
				bindKeys(Control.DOWN, getSaveKeys(Control.DOWN));
				bindKeys(Control.LEFT, getSaveKeys(Control.LEFT));
				bindKeys(Control.RIGHT, getSaveKeys(Control.RIGHT));
	...
	private static function getSaveKeys(type:Control)
	{
		return KeyManager.getNoteKeys().get(type);
	}

	public static function saveControls_start(reset:Bool)
	{
		if(FlxG.save.data.upKey == null || reset)
            FlxG.save.data.upKey = ["W", "UP"];
        if(FlxG.save.data.downKey == null || reset)
            FlxG.save.data.downKey = ["S", "DOWN"];
        if(FlxG.save.data.leftKey == null || reset)
            FlxG.save.data.leftKey = ["A", "LEFT"];
        if(FlxG.save.data.rightKey == null || reset)
            FlxG.save.data.rightKey = ["D", "RIGHT"];
		if(reset)
            trace("RESETED CONTROLS!");
        else 
            trace("CHECKED CONTROLS..");
		FlxG.save.flush();
	}
}
